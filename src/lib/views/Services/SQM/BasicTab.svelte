...
<div class={cn("flex", "flex-col", "gap-4")}>
  ...
  <NetworkSelect label={t("Interface name")} bind:value={q.interface} placeholder={t("Select...")} />
  ...
  <div class={cn("grid", "grid-cols-2", "gap-3")}>
    ...
    <Input label={t("Upload speed (egress)")} bind:value={q.upload} placeholder="0" type="number" description={t("Upload speed (kbit/s) (egress) set to 0 to disable egress shaping selectively")} />
  </div>
