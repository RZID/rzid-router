<script lang="ts">
  ...
  import { readFile, statFile, writeFile } from "../../../api/ubus";
  ...
  const LOCAL_FILE = "/etc/banip/banip.allowlist";
  ...
  <div class={cn("shrink-0")}>
    <div>
      <h1 class={cn("text-lg", "font-semibold", "text-white")}>
        {trans("Edit Allowlist")}
      </h1>
      <p class={cn("text-sm", "mt-0.5", "text-muted")}>
        {trans("This is the local banIP allowlist.")}<br />
        {trans(
          "Add only exactly one MAC/IPv4/IPv6 address or domain name per line.",
        )}
      </p>
    </div>
  </div>
  <div class={cn("flex", "items-center", "gap-3")}>
    ...
    <button
      ...
      class={cn(
        ...
        "cursor-pointer",
        saving
          ? "text-muted bg-surface-3 border-transparent"
  ...
  {:else}
    <div class={cn("glass", "p-5", "rounded-xl", "animate-slide-up")}>
      <Textarea
        bind:value={content}
        rows={25}
        placeholder={trans("Enter MAC/IPv4/IPv6 addresses, one per line...")}
      />
    </div>
  {/if}
</div>
