<script lang="ts">
  ...
  import {
    Play, Square, RotateCw, ToggleLeft, ToggleRight, Search, Power,
  } from "@lucide/svelte";
  ...
  async function loadScripts() {
    ...
    try {
      ...
      scripts = (all?.entries || [])
        .filter(e => e.type === "file" && e.name !== "README" && !e.name.startsWith("."))
        .map(e => ({
          name: e.name,
  ...
  const filtered = $derived(
    filter
      ? scripts.filter(s => s.name.toLowerCase().includes(filter.toLowerCase()))
      : scripts,
  );

  async function doAction(name: string, action: string, e: Event) {
    e.stopPropagation();
...
<div class={cn("p-6", "flex", "flex-col", "h-dvh", "gap-4", "animate-fade-in")}>
  ...
  <div class={cn("shrink-0", "flex", "items-center", "gap-3")}>
    <div class={cn("w-9", "h-9", "rounded-xl", "bg-accent/10", "flex", "items-center", "justify-center", "ring-1", "ring-accent/20", "shrink-0")}>
      <Power size={16} class="text-accent" />
    </div>
    <div>
      <h1 class={cn("text-lg", "font-semibold", "text-white", "tracking-tight")}>{trans("Startup")}</h1>
      <p class={cn("text-xs", "text-muted")}>{trans("Manage init scripts and services on startup")}</p>
    </div>
  </div>
  ...
  {:else if filtered.length === 0}
    <div class={cn("flex-1", "flex", "items-center", "justify-center")}>
      <p class={cn("text-sm", "text-muted")}>{trans("No init scripts found")}</p>
    </div>
  {:else}
        ...
        <table class={cn("w-full", "text-xs")}>
          <thead class={cn("sticky", "top-0", "z-10")}>
            <tr class={cn("border-b", "border-border", "bg-surface-2/95", "backdrop-blur-sm")}>
              <th class={cn("text-left", "p-3", "text-[10px]", "uppercase", "text-muted", "font-semibold", "tracking-wider")}>{trans("Init Script")}</th>
              <th class={cn("text-center", "p-3", "text-[10px]", "uppercase", "text-muted", "font-semibold", "tracking-wider", "hidden", "sm:table-cell")}>{trans("Status")}</th>
              <th class={cn("text-center", "p-3", "text-[10px]", "uppercase", "text-muted", "font-semibold", "tracking-wider", "hidden", "md:table-cell")}>{trans("Priority")}</th>
              <th class={cn("text-right", "p-3", "text-[10px]", "uppercase", "text-muted", "font-semibold", "tracking-wider")}>{trans("Actions")}</th>
            </tr>
          </thead>
          <tbody>
            {#each filtered as s, i}
              <tr class={cn(
                ...
                <td class={cn("p-3", "text-center", "hidden", "sm:table-cell")}>
                  {#if s.enabled}
                    <span class={cn("inline-flex", "items-center", "gap-1", "px-2", "py-0.5", "text-[10px]", "font-medium", "rounded", "bg-accent/8", "text-accent")}>
                      {trans("Enabled")}
                    </span>
                  {:else}
                    <span class={cn("inline-flex", "items-center", "gap-1", "px-2", "py-0.5", "text-[10px]", "font-medium", "rounded", "bg-surface-3", "text-muted")}>
                      {trans("Disabled")}
                    </span>
                  {/if}
                </td>
                <td class={cn("p-3", "text-center", "font-mono", "text-muted", "hidden", "md:table-cell")}>
                  {s.priority || "-"}
                </td>
                <td class={cn("p-3", "text-right")}>
                  <div class={cn("inline-flex", "items-center", "gap-1")}>
                    {#each [
                      { action: "start", icon: Play, label: "Start", color: "accent" as const },
                      { action: "stop", icon: Square, label: "Stop", color: "danger" as const },
                      { action: "restart", icon: RotateCw, label: "Restart", color: "accent" as const },
                    ] as btn}
                      <button
                        onclick={(e) => doAction(s.name, btn.action, e)}
                        disabled={actionFeedback[s.name] !== undefined && actionFeedback[s.name] !== "" && actionFeedback[s.name] !== "done" && actionFeedback[s.name] !== "failed"}
                        title={btn.label}
                        class={cn(
                          ...
                          actionFeedback[s.name] === btn.action && "animate-pulse",
                        )}
                      >
                        <btn.icon size={12} />
                      </button>
                  ...
                  {#if actionFeedback[s.name] === "done"}
                    <span class={cn("text-[10px]", "text-accent", "ml-1")}>✓</span>
                  {:else if actionFeedback[s.name] === "failed"}
                    <span class={cn("text-[10px]", "text-danger", "ml-1")}>✗</span>
                  {/if}
                </td>
              </tr>
            {/each}
