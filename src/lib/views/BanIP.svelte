<script lang="ts">
  ...
  import Overview from "./banip/Overview.svelte";
  import Allowlist from "./banip/Allowlist.svelte";
  import Blocklist from "./banip/Blocklist.svelte";
  import CustomFeed from "./banip/CustomFeed.svelte";
  import SetReport from "./banip/SetReport.svelte";
  import FirewallLog from "./banip/FirewallLog.svelte";
  import ProcessingLog from "./banip/ProcessingLog.svelte";

  let locale = $state(getLocale());
  let trans = $derived.by(() => { locale; return (k: string) => _t(k); });
  $effect(() => onLocaleChange(() => { locale = getLocale(); }));

  type SubId = "overview" | "allowlist" | "blocklist" | "feeds" | "setreport" | "fwlog" | "proclog";
...
<div class={cn("p-6", "flex", "flex-col", "min-h-0", "animate-fade-in", "gap-4")}>
  <div class={cn("shrink-0")}>
    <TabBar tabs={subPages} active={activeSub} onchange={(id) => activeSub = id as SubId} />
  </div>
