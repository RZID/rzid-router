<script lang="ts">
  ...
  const pollInfo = async () => {
    ...
    if (rt) {
      ...
      infoParseErr++;
      if (infoParseErr >= 6) clearInterval(pollTimer); // ~12s
    }
  };
...
<div class={cn("p-6", "flex", "flex-col", "min-h-0", "animate-fade-in", "gap-5")}>
  ...
  <!-- Sub-tabs -->
  <div class={cn("shrink-0", "overflow-x-auto")}>
    <div class={cn("flex", "gap-1", "p-0.5", "w-fit", "border", "rounded-lg", "bg-surface-2", "border-border")}>
      {#each tabsList as t}
        <button
          class={cn("px-3", "py-1.5", "text-xs", "rounded-md", "font-medium", "transition-all", "cursor-pointer", "whitespace-nowrap", tab === t.id ? "bg-accent text-surface" : "bg-transparent text-muted")}
          onclick={() => (tab = t.id)}
        >{t.label}</button>
      {/each}
    </div>
  ...
  <!-- Action buttons -->
  <div class={cn("shrink-0", "flex", "items-center", "gap-2")}>
    <button
      onclick={() => handleAction("stop")}
      disabled={actionBusy}
      class={cn("inline-flex", "items-center", "gap-1.5", "px-3", "py-1.5", "text-xs", "rounded-lg", "font-medium", "transition-all", "duration-150", "cursor-pointer", "select-none", "border", "bg-danger/10 text-danger border-danger/20 hover:bg-danger/20", "disabled:opacity-30")}
    >
      <Square size={14} />
      {trans("Stop")}
    </button>
    <button
      ...
      disabled={actionBusy}
      class={cn("inline-flex", "items-center", "gap-1.5", "px-3", "py-1.5", "text-xs", "rounded-lg", "font-medium", "transition-all", "duration-150", "cursor-pointer", "select-none", "border", "text-accent bg-accent/10 border-accent/20 hover:bg-accent/20", "disabled:opacity-30")}
    >
      <RotateCcw size={14} />
      {trans("Save & Reload")}
    </button>
