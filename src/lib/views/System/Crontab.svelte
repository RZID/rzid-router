...
<div class={cn("p-6", "flex", "flex-col", "h-dvh", "gap-4", "animate-fade-in")}>
  ...
  <div class={cn("shrink-0", "flex", "items-center", "gap-3")}>
    ...
    >
      <Clock size={16} class={cn("text-accent")} />
    </div>
    <div>
      <h1
  ...
  >
    <div class={cn("flex", "items-center", "gap-2")}>
      {#if feedback === "saved"}
        <span class={cn("text-xs", "text-accent")}
          >✓ {trans("Contents have been saved.")}</span
        >
      {:else if feedback === "error"}
        <span class={cn("text-xs", "text-danger")}
          >✗ {trans("Failed to save")}</span
        >
      {:else if dirty}
