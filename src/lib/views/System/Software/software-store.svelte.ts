...
export const getIsApk = () => isApk;
export const getLoading = () => loading;
export const getOperating = () => operating;
export const getDiskTotal = () => diskTotal;
export const getDiskFree = () => diskFree;
export const getTab = () => tab;
export const getFilter = () => filter;
export const getI18nFilter = () => i18nFilter;
...
export const getPageOffset = () => pageOffset;
export const getSortField = () => sortField;
export const getSortDir = () => sortDir;
export const getModalOpen = () => modalOpen;
export const getModalTitleHtml = () => modalTitleHtml;
export const getModalBody = () => modalBody;
export const getModalActions = () => modalActions;

export let available: PkgMap = $state({ providers: {}, pkgs: {} });
export let installed: PkgMap = $state({ providers: {}, pkgs: {} });
...
const _pkgMgr = $derived(isApk ? "APK" : "OPKG");
export const getPkgMgr = () => _pkgMgr;

export const closeModal = () => {
  modalOpen = false;
};

export const showModal = (titleHtml: string, body: any, actions?: any[]) => {
...
export const showDetail = (name: string) => {
  ...
  let extraRows = "";
  if (avail.section)
    extraRows += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Section")}</span><span class="text-fg font-mono">${avail.section}</span></li>`;
  if (avail.architecture)
    extraRows += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Arch")}</span><span class="text-fg font-mono">${avail.architecture}</span></li>`;
  if (avail.repository)
    extraRows += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Repository")}</span><span class="text-fg font-mono">${avail.repository}</span></li>`;
  if (pkg.installtime)
    extraRows += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Installed")}</span><span class="text-fg font-mono">${pkg.installtime.toLocaleString()}</span></li>`;

  let badges = "";
  if (avail.essential)
    badges += `<span class="text-[10px] px-1.5 py-0.5 rounded font-medium bg-accent/8 text-accent">${t("Essential")}</span>`;
  if (avail["auto-installed"])
    badges += `<span class="text-[10px] px-1.5 py-0.5 rounded font-medium bg-warn/8 text-warn">${t("Auto")}</span>`;
  if (pkg.hold || avail.hold)
    badges += `<span class="text-[10px] px-1.5 py-0.5 rounded font-medium bg-danger/8 text-danger">${t("Hold")}</span>`;
  if (badges)
    extraRows += `<li class="flex items-center gap-1"><span class="text-muted w-20 shrink-0">${t("Flags")}</span>${badges}</li>`;
  ...
  if (i18nPkgs.length) {
    i18nSection = `<div class="mt-3 pt-3 border-t border-border"><label class="flex items-center gap-2 text-xs cursor-pointer"><input type="checkbox" checked id="i18n-cb" data-pkgs="${i18nPkgs.join(" ")}" class="accent-(--accent)" /> <span class="text-muted">${t("Install suggested translation packages as well")}</span></label></div>`;
  }

  overwriteSection = `<div class="mt-2"><label class="flex items-center gap-2 text-xs cursor-pointer"><input type="checkbox" id="overwrite-cb" class="accent-(--accent)" /> <span class="text-muted">${t("Allow overwriting conflicting package files")}</span></label></div>`;

  if (avail.description)
    desc = `<div class="mt-3 pt-3 border-t border-border"><h5 class="text-xs font-semibold text-white mb-1">${t("Description")}</h5><p class="text-xs text-muted leading-relaxed">${avail.description}</p></div>`;

  let html = `<ul class="space-y-1.5 text-xs" style="padding-left:0">`;
  html += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Version")}</span><code class="text-accent bg-accent/8 px-2 py-0.5 rounded font-mono">${pkg.version || "-"}</code></li>`;
  html += `<li class="flex items-center gap-2"><span class="text-muted w-20 shrink-0">${t("Size")}</span><span class="text-fg font-mono">${size}</span></li>`;
  html += extraRows;
  html += tree;
  html += providesHtml;
  html += rdepHtml;
  ...
  const act: any[] = [{ label: t("Cancel"), action: closeModal }];
  ...
  showModal(t("Package: {name}").replace("{name}", name), html, act);
};

export function handleI18nChange(v: "lang" | "all" | "none") {
  i18nFilter = v;
  applyDisplay();
}

export const handleRemoveModal = (name: string) => {
  ...
  html += `<label class="flex items-center gap-2 text-xs cursor-pointer"><input type="checkbox" checked id="autoremove-cb" ${apkDisabled} class="accent-(--accent)" /> <span class="text-muted">${t("Automatically remove unused dependencies")}</span></label>`;
  html += `</div>`;

  showModal(t("Remove {name}").replace("{name}", name), html, [
...
export const handleManualInstall = () => {
  ...
  if (val.indexOf("/") !== -1) {
    warning = `<p class="text-xs text-muted">${t("Installing packages from untrusted sources is a potential security risk! Really attempt to install {pkg}?").replace("{pkg}", `<code class="text-accent bg-accent/8 px-1.5 py-0.5 rounded font-mono">${val}</code>`)}</p>`;
  } else if (!available.providers[val]) {
    warning = `<p class="text-xs text-muted">${t("The package {pkg} is not available in any configured repository.").replace("{pkg}", `<code class="text-accent bg-accent/8 px-1.5 py-0.5 rounded font-mono">${val}</code>`)}</p>`;
    installBtn = null as any;
  } else {
    warning = `<p class="text-xs text-muted">${t("Really attempt to install {pkg}?").replace("{pkg}", `<code class="text-accent bg-accent/8 px-1.5 py-0.5 rounded font-mono">${val}</code>`)}</p>`;
  }

  showModal(t("Manually install package"), warning, [
    { label: t("Cancel"), action: closeModal },
...
export const handleUpload = () => {
  showModal(
    t("Upload Package"),
    `<div class="flex flex-col items-center gap-3 py-4"><p class="text-xs text-muted">${t("Upload an .ipk or .apk file to install.")}</p><input type="file" id="upload-file" accept="${isApk ? ".apk" : ".ipk,.ipk"}" class="text-xs text-muted file:mr-2 file:px-3 file:py-1.5 file:text-xs file:rounded-lg file:font-medium file:border file:border-accent/20 file:bg-accent/8 file:text-accent file:cursor-pointer hover:file:bg-accent/15" /></div>`,
    [
      { label: t("Cancel"), action: closeModal },
      { label: t("Upload & Install"), action: doUpload, accent: true },
    ],
  );
};
...
export const handleConfig = () => {
  ...
  (async () => {
    try {
      ...
      showModal(`${_pkgMgr} Configuration`, body, [
...
const execPkgCmd = (cmd: string, pkg?: string) => {
  ...
  showModal(
    `Executing ${_pkgMgr}`,
    `<div class="flex flex-col items-center gap-3 py-6"><div class="relative"><RotateCw size={28} class="text-accent animate-spin" /><div class="absolute inset-0 rounded-full animate-ping bg-accent/20"></div></div><span class="text-xs text-muted">Waiting for ${_pkgMgr} ${cmd} ${name} to complete…</span></div>`,
    [],
  );
  execCmd(cmd, name);
};

export const handlePkg = (action: string, name: string) => {
  closeModal();
  const autoremove =
...
const execPkgCmdRaw = async (argv: string[], action: string) => {
  ...
  showModal(
    `Executing ${_pkgMgr}`,
    `<div class="flex flex-col items-center gap-3 py-6"><div class="relative"><RotateCw size={28} class="text-accent animate-spin" /><div class="absolute inset-0 rounded-full animate-ping bg-accent/20"></div></div><span class="text-xs text-muted">Waiting for ${_pkgMgr} ${action} to complete…</span></div>`,
    [],
  );
  ...
  html += `<span class="text-sm font-medium">${hasErr ? `${_pkgMgr} ${action} failed` : `${_pkgMgr} ${action} completed`}</span></div>`;
  if (res?.stdout)
...
export const switchTab = (t: typeof tab) => {
  if (t === tab) return;
  tab = t;
  applyDisplay();
};

const _pagedRows = $derived(
  currentRows.slice(pageOffset, pageOffset + PAGE_SIZE),
);
const _totalPages = $derived(Math.ceil(currentRows.length / PAGE_SIZE));
const _currentPage = $derived(Math.floor(pageOffset / PAGE_SIZE) + 1);
export const getPagedRows = () => _pagedRows;
export const getTotalPages = () => _totalPages;
...
export const toggleSort = (field: typeof sortField) => {
  if (sortField === field) {
...
const _diskUsed = $derived(diskTotal - diskFree);
const _diskPct = $derived(
  diskTotal > 0 ? Math.round((_diskUsed / diskTotal) * 100) : 0,
);
export const getDiskUsed = () => _diskUsed;
export const getDiskPct = () => _diskPct;

const _tabCounts = $derived.by(() => {
  let i18nRegex: RegExp | null = null;
  switch (i18nFilter) {
    case "all":
      i18nRegex = /^luci-i18n-/;
...
export const getTabCounts = () => _tabCounts;

const formatSize = (size: number): string => {
  return fmtBytes(size).replace(" ", "");
};

export const doMount = () => {
