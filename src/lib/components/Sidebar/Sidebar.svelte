<script lang="ts">
  ...
  // Props
  let { onlogout, onnavigate, active = "dashboard" }: Props = $props();

  // States
  let locale = $state(getLocale()),
    expanded = $state<Record<string, boolean>>({ Status: true });
      ...
      <div>
        ...
        {#if expanded[cat.label]}
          <div
            ...
            class={cn("ml-4 mt-0.5 space-y-0.5 overflow-hidden")}
          >
            {#each cat.children as item}
              ...
              >
                <span class={cn("font-medium", "text-xs")}
